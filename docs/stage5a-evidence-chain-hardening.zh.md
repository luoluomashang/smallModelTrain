...
Stage 5A 的目标，是把训练、推理、评分和审阅结果变成可追溯的证据链：训练前检查同时产出人读 Markdown 和机器读 JSON，eval 生成保留原始输出，评分默认看 `raw_output`，规则投影不会伪装成独立审阅，正式 SFT 只吃已批准或冻结的章节卡。
...
- 适用于 formal SFT 前的门禁复查：formal SFT 默认拒绝 draft cards。只有 `approval_status` 为 `approved` 或 `frozen`，且带有 `style_contract_id`、64 位十六进制 `style_contract_sha256` 等 style contract metadata 的卡，才允许进入 formal。
