...
当前实现里常见字段名可能是 `id`、`chapter_title`、`char_count_zh`，含义和上面的简化示例一致：它们都在描述一条章节记录。
...
当前训练构造代码主要用 `id` 作为连接键，把章节卡和清洗后的章节正文配起来。`split` 很重要：只有 train 样本应该进入 SFT，eval 样本要保留到评测时再用。
...
当前评分实现还会保存 `hard_gate_pass`、`failure_types`、`char_count_zh`、`must_include_coverage`、`repeated_ngram_ratio` 等字段，方便后面定位问题。
