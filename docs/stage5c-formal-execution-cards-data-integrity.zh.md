...
Stage 5C 把 `data_cards/chapter_cards.jsonl` 里的草稿章节卡升级为可审阅、可批准、可哈希、可追踪的 `ChapterExecutionCard` 资产。正式训练不再把普通草稿卡当成 formal 证据，而是要求每一条训练样本都能追溯到一张正式卡、一个源章节、一个 StyleContract 和一个稳定 split。
...
人工审阅后，只有确认通过的卡才能改为 `approved` 或 `frozen`，并且修改审批状态、审阅记录或任何正式字段后，都要重新计算 `card_sha256`。formal SFT 读取的是审批冻结后的 `data_cards/chapter_execution_cards_approved.jsonl`。

## Formal SFT 命令

formal SFT 构建数据时使用 approved/frozen 的正式卡、approved/frozen 的 StyleContract JSON，并写出 dataset manifest：
...
`--dataset-manifest-output` 写出的 manifest 是数据集证据，不替代训练后的 `run_manifest.json`。前者证明这批 SFT 数据由哪些章节、卡、split 和 StyleContract 构成；后者证明某次训练命令实际消耗了哪些输入。

## Formal 门禁检查

进入 formal SFT 前，至少要检查：

- 每个 train/A 章节恰好有一张 `approved` 或 `frozen` 的 `ChapterExecutionCard`。
...
- prompt 不泄漏目标正文、source_text、validation/sealed 文本片段或禁止引用的长片段。
- dataset manifest 记录 dataset、card、chapter、split、StyleContract 的 hash provenance，能反查每一行 SFT 数据的来源。

任一项不通过，都应阻断 formal SFT，而不是降级为草稿路径。

## Sealed Split 边界

Stage 5C 的 sealed split 是保留证据边界。`sealed` 章节不能进入训练，不能用于调参，也不应用来挑 prompt、挑阈值或修卡。formal SFT 只能使用 train 侧符合门禁的章节；validation 可以服务开发期比较和预算调参；sealed 只留给后续最终证明。

dataset manifest 中的 split 信息用于证明 train、validation、sealed 没有重叠，并且 formal 数据集没有把 sealed 内容带入训练。
...
- 不证明 sealed eval 已经通过；它只保证 sealed 边界没有被 formal SFT 训练数据消耗。
